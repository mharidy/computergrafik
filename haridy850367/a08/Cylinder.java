	...
	public Hit intersect(Ray r) {
		...
		if (y1 < -height) {
			if (y2 < -height)
				...
				return new Hit(th, hi, nor, material);
			}
		}
		if (y1 >= -height && y1 <= height) {
			...
			Vec3 hi1 = r.pointAt(t1);
			Vec3 nor = divide(subtract(hi1, center), radius);
			Hit hit = new Hit(t1, hi1, nor, material);
			return hit;
		...
		if (y1 > height) {
			...
			Vec3 hi = r.pointAt(th);
			Vec3 nor = divide(subtract(hi, center), radius);
			Hit hit = new Hit(th, hi, nor, material);
			// return null;
			return hit;
