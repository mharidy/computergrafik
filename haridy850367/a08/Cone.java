...
public class Cone implements Shape {
	...
	public Hit intersect(Ray r) {
		...
		double b = (2 * newOrigin.x * r.normalizedDir.x) - (2 * newOrigin.y * r.normalizedDir.y)
				+ (2 * newOrigin.z * r.normalizedDir.z);
		...
		if (y1 < -height) {
			if (y2 < -height)
				...
				Hit hit = new Hit(th, treffer, normal, mat);
				return hit;
			}
		}
		if (center.y - height < r.pointAt(t1).y && r.pointAt(t1).y < center.y) {
			...
			Hit hit = new Hit(t1, hi, normalize(normal), mat);
