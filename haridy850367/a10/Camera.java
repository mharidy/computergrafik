...
public class Camera {
    ...
    public Ray generateRay(double x, double y){
        //return new Ray(new Vec3(0,0,0), new Vec3(x - (int)(width / 2), (int)((height/2) - y), - (int)((width / 2) / Math.tan(Math.toRadians(angle / 2)))), 0, Double.POSITIVE_INFINITY);
        ...
        Vec3 temp = new Vec3(x - (int)(width / 2), (int)((height/2) - y), - (int)((width / 2) / Math.tan(Math.toRadians(angle / 2))));
        
        Vec3 dir = xrot.transformDirection(temp);
        dir = yrot.transformDirection(dir);
        dir = zrot.transformDirection(dir);
        
        return new Ray(transform.transformPoint(new Vec3(0,0,0)), dir, 0, Double.POSITIVE_INFINITY);
    }
}
