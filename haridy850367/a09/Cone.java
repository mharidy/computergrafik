...
public class Cone implements Shape{
    ...
    public Cone(Vec3 center, double radius, double height, Material material){
        this.center = center;
        this.radius = radius;
        this.height = height;
    ...
    public Hit intersect(Ray r) {
        ...
        if(y1 < -height) {
            if(y2 < -height) return null;
                ...
                Hit hit= new Hit(th, treffer, normal, material);
                return hit;
            }
        }
        if(center.y - height < r.pointAt(t1).y && r.pointAt(t1).y < center.y) {
            ...
            Hit hit= new Hit(t1, hi, normalize(normal), material);
            return hit;
        }
